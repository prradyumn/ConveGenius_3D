# ConveGenius — Mobile Charging Repair Simulator (three.js runtime)

The browser runtime for the BC.1 / BC.2 / BC.3 Mobile Charging Repair course.
Built against the finished v5 asset package; **no asset was remodelled**.
...
Assets are copied into `public/assets/` at setup. To refresh them from the
package:

```bash
cp ../ConveGenius_MobileRepair_3D_v5/glb/*_LOD0.glb public/assets/glb/
cp ../ConveGenius_MobileRepair_3D_v5/components.json public/assets/data/
```
...
### 1. `ANIM_B10_PEEL` works. `reports/GATE_CHECK.json` is wrong about it.
...
- `MAT_POLYIMIDE_V2` appears in `reports/GATE_CHECK.json` but the shipped binary
