...
Start with **`THREEJS_BUILD_PROMPT.md`** and **`components.json`**.
...
```
glb/                  6 files, 5.9 MB   the deliverable — glTF 2.0 binary, Draco compressed
tex/                 15 files, 1.9 MB   source texture maps (already embedded in the glb)
build_scripts/       14 files, 240 KB   every asset is generated by script; these ARE the source
blend/                6 files, 1.1 MB   Blender working files
renders/             42 files,  57 MB   shot list, 1500–2400px transparent PNG
reports/             10 files           per-asset build + verification JSON
docs/                                   status docs, the original asset register, contact sheets
components.json                         1,109 labelled components — the runtime's label API
THREEJS_BUILD_PROMPT.md                 the implementation brief
```
...
```
blender-launcher.exe --python build_scripts\run_all.py -- --cg-queue build_b05_port.py,build_b10_b11_ifc.py,build_b40_joints.py,build_b02_mainboard.py,build_b28_hotair.py,build_b02_assembly.py,texture_pass.py
```
